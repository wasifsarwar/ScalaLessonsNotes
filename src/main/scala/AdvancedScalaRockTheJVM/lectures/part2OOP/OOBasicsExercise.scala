...
object OOBasicsExercise extends App{
  ...
  if (novel.isWrittenBy(wasif)) println(s"yes, ${wasif.apply()} wrote ${novel.novelName}")
  else (println(s"no, this author didn't write ${novel.novelName}"))
  println(s"the author of this book ${novel.author.apply()} is ${novel.authorAge()} years old}")
  novel.copy(2020)
