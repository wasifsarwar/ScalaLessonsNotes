...
object WasifFunctionalProgramming extends App {
  ...
  class Person(name: String) {
    def apply(age: Int): Unit = println(
      s"My name is $name and I have aged $age years"
    )
  }
  ...
  // shorter syntax
  val filteredListShort = List(1, 3, 55, 23, 12, 95).filter(
    _ <= 58
  ) // notice removing x => with _ followed with boolean condition
  ...
  val pairs =
    numbers.flatMap(number => letters.map(letter => s"$number-$letter")).sorted
  ...
  val tail = aList.tail
  val aPrependedList =
    0 :: aList // :: refers to aPrepended list adds before head
  val aAppendedList = 0 +: aList :+ 6 //  +: adds at head, :+ adds at tail

  // Sequences
  val aSequence =
    Seq(1, 2, 3, 4, 5) // basically seq.apply() Seq is a abstract type

  val accessedElement = aSequence(
    2
  ) // applying a sequence to an index will return that element in that index
  ...
  /**    - sets = no duplicates.
    ...
    */
  val aSet = Set(1, 2, 3, 4, 1,
    2) //only has unique elements, result should be Seq(1,2,3,4)
  ...
  val mapMap = aMap.map { case (_, value) =>
    value == 3
  }

  val findValue = aMap.find { case (_, value) =>
    value == 3
  }
