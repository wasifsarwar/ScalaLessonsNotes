...
object ObjectOrientation extends App {
  ...
  aDeclaredAnimal
    .eat() //the most derived method (as in it'll call from Dog) is called in runtime. In compiled time, compiler only knows it comes from Animal, but in runtime the method from the most derived class will be executed

  // abstract class
  abstract class WalkingAnimal {
    val hasLegs =
      true //by default public, can restrict that by using private/protected
    def walk(): Unit
  }
  ...
  class Crocodile extends Animal with Carnivore with Philosopher {
    override def eat(animal: Animal): Unit = println(
      "I am a crocodile eating you, animal"
    )
  ...
  val dinosaur = new Carnivore {
    override def eat(animal: Animal): Unit = println(
      "I am dinosaur I can eat anything RAWWR"
    )
  }
